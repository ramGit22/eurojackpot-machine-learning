...
# Load the data
data = pd.read_csv('C:\\Users\\karki\\Desktop\\lottery-result\\results.csv')  # replace with your csv file path
...
extra_targets = ['extra'+str(i) for i in range(1, 11)]
...
for target in main_targets:
    model = RandomizedSearchCV(XGBClassifier(use_label_encoder=False), param_distributions, n_iter=5)
...
# Predict the cluster for the next draw
next_draw_scaled = scaler.transform([[31, 2023]])  # replace with the week and year of the next draw
